...
describe("primary-flow chrome lock", () => {
  ...
  it("locks Beat 1 Keys / Configuration, hidden URL, ?, Open/Edit Select Delete", () => {
    ...
    assert.match(html, /id="advanced-rtmp"/);
    assert.match(html, /id="rtmp_url"[^>]*type="text"/s);
    const advanced = html.slice(html.indexOf('id="advanced-rtmp"'), html.indexOf('id="key-list"'));
    assert.match(advanced, /id="rtmp_url"/);
    ...
    assert.match(html, /id="key-list"/);
    assert.match(html, /id="add-key-btn"[^>]*>Add</s);
    assert.match(html, /id="save-btn"[^>]*>Save</s);
    assert.match(main, /retransApi\.saveKey/);
    assert.match(main, /putKeyBody/);
  ...
  it("Start sends source_url + key_id only; no clip UI; Vite loopback 8788", () => {
    ...
    assert.match(api, /\/api\/live\/keys/);
  ...
  it("boot routes GET keys → Beat 2; poll fail stays Idle; first Save sticks", () => {
    assert.match(main, /retransApi\.listKeys\(\)/);
    assert.match(main, /applyKeysBoot/);
